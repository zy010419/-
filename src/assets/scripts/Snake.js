...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.speed = 5; // 蛇每秒走五格
        this.direction = -1; // -1表示没有zhiling，0、1、2、3表示上右下左
        this.status = "idle"; // idle表示闲置，move表示正在移动，die表示死亡
    ...
    render() {
        ...
        for (const cell of this.cells) {
            ctx.beginPath();
            ctx.arc(cell.x * L, cell.y * L, (L / 2) * 0.8, 0, Math.PI * 2); // 前两个参数为圆的中心坐标, 第三个是圆的半径, 后面两个参数是圆的起始位置
            ctx.fill();
        }
